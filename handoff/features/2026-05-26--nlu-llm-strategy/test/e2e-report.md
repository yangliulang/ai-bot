...
- 环境：
  ...
  - 后端 API：回归 `uv run pytest tests/test_nlu_llm_strategy.py -q` → **9 passed**
...
|--------|------|
| 主流程页面可访问 | N/A |
| Admin `intentNluUseLlm` 开关 | N/A（后续 `/ai-settings` FE 包） |
| 意图 NLU 策略（HTTP/TG） | 已由 API TC-02～TC-07 覆盖 |
...
- `design/ui-review.md` 仍为占位；建议下一 Chat **`/pipeline-designer-review 2026-05-26--nlu-llm-strategy`** 记 **N/A** 后 `ui_reviewed`。
- Telegram 全场景 narrate 扩面见 `2026-05-26--telegram-llm-narrate`，不在本包 E2E 范围。
