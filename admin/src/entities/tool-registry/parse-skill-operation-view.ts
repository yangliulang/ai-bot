 ...
 * 作者: Cursor Agent
 * 日期: 2026-05-26
 * 修改功能: skill-spec Markdown §1～§6 解析（对齐 product-doc parseSkillOperationView）
 */
...
export function parseConfirmRules(prose: string): ConfirmRule[] {
  ...
  for (const para of paragraphs) {
    ...
    for (const line of lines) {
      ...
      if (labeled) {
        rules.push({ title: stripMd(labeled[1]!), body: stripMd(labeled[2] ?? '') })
        continue
      }
...
export function parseSkillOperationView(body: string): SkillOperationView {
  ...
  return {
    ...
    sectionNotes: { confirm: confirmProse.slice(0, 800) },
  }
}
